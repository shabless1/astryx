...
> - **STEP 6 DONE 2026-09-13** (worker `37412ee`): `src/shape/{fields,calibration,pick,assert}.ts`. Built as an **allowlist** — responses are assembled by naming what may leave, never by deleting from the engine's output. **basic** = carrier + signals/states + 6-Sense Protocol + SOAP + session + prescriptions + safety notes. **premium** adds clinical depth (mechanism, nerve plexus, dosing), polarity detail, the diagnostic layer, reflex placements. **Neither**: the `remedyPolarity` corrective rows (crown jewel #1), the scores/weights, the routing keys, Lotus Spectrum, starter kits, the esoteric botanical voice. `assert.ts` is a name-based backstop independent of the shaping code — it trips on **75 fields** in an unshaped protocol. 89 tests, parity 197/197, workerd exact.
> - **FOUND, NOT FIXED — the app's THIRD door.** `/api/protocol` returns `{ ...protocol, sacredLayer: shaped, prescriptions: shaped }`, so **everything else ships raw to any signed-in user**: `polarityResults[].protocol` + `dominantPolarity.protocol` (the whole remedyPolarity corrective row — crown jewel #1, enumerable in a few dozen requests), `polarityResults[].scores`/`.confidence`, `activePlanets[]` raw weights, `diagnostic.symptomRouting[].matched*` routing keys, and `prescriptions[].mineral.dosing` on the free surface. Same class as `1e68aa9` and `9576ecf`; predates this work; the Worker does not have it. **Not a quick delete** — `ResultsScreen.tsx` (~1594-1603, ~1764-1885) and `SessionScreen.tsx:253` render `corrective_direction`, `regulator_planets`, `avoid`, `color_palette`. Needs display-field shaping, the same pattern as the first two doors. Spawned as its own task; `astryx-core-worker/src/shape/` is the worked reference.
> - **NEXT:** step 7 (HTTP layer — router, CORS, per-caller metering in a Durable Object or KV; the app's in-memory rate limiter does not survive across isolates), then 8 (the x402 door).
>
