...
2. **TWO frequency systems, kept separate.** (a) The **PLANETARY protocol** — forks, chamber audio, planetary anchors — comes from `planetary-anchors.json` and `sacredTones_nervousSystem.json` only, sourced from Hans Cousto's Law of the Octave. Do NOT use Solfeggio for the planetary protocol. (b) The **CHAKRA layer** uses the standard chakra **Solfeggio** tones (Root 396 · Sacral 417 · Solar Plexus 528 · Heart 639 · Throat 741 · Third Eye 852 · Crown 963), drawn from `solfeggio-overlays.json` (which also feeds the aspect harmonic overlay). The chakra Hz live in the `CHAKRAS` array in `BodyMap.tsx`. (SHA directive 2026-06-20 — corrected from the old C-scale 256–480 values.) **THE METAL IS THE SOURCE OF TRUTH (SHA ruling 2026-09-11).** Where a shipped Sacred Tones fork is engraved with a frequency, the app MUST match the engraving, because the practitioner is holding the fork and reading the screen at the same time. Two values were aligned to the physical sets: **Uranus 207.36 → 207.33** and **Neptune 211.44 → 211.45**. These now differ by hundredths of a hertz from the strict Cousto derivation; that is deliberate and must not be "corrected" back. If a future fork run is engraved differently, the app follows the metal again.
