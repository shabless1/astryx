...
**0.2 — BUILT + LIVE 2026-09-09 (commit 235de6c, migration `20260909000000_outcome_capture`, deploy astryx-929odfslg).** Columns on `ChamberSession` (RLS on): energyBefore/After, carrierPlanet, signalState, forkSequence, intention, chartHash, standardVersion, outcome (whitelisted felt-state), outcomeAt. `POST /api/sessions` resolves readingId+chartHash server-side from the latest Reading and stamps `CALIBRATION_STANDARD_VERSION`; `PATCH /api/sessions/:id` is owner-scoped + write-once. Shaping lives in `src/lib/outcomeCapture.ts` (8 tests fence the whitelist; free text never stored). Every signed-in session from today forward counts. Original spec follows.
...
| # | Deliverable | Notes |
|---|---|---|
| 1.1 | **DONE 2026-09-10 — LIVE at myastryx.com/standard** (commit 6f3b24c). SHA approved voice (expanded metaphysical register), IP boundary §7, fork count = 12; §4 now carries the 8 Clairs as the receptive side of the six channels. **Publish the Astryx Calibration Standard** — `ASTRYX_CALIBRATION_STANDARD_v0.md` → a public page `myastryx.com/standard`. | The method, the rules (never-amplify, Earth bookends, the ladder, the 6 senses), the *existence* of the mappings, the sources (Cousto, classical). **Not the tables.** Sell the output, never the dataset. |
